...
export type MangaEpisode = {
	...
	Title: string;
	ImageUrl: string[];
	/** ImageUrl と同じ並び順の画像サイズ。レイアウトシフト防止用。 */
	ImageSize: MangaImageSize[];
};
...
function buildEpisodes(): MangaEpisode[] {
	...
	for (const [path, mod] of Object.entries(mangaImages)) {
		...
		const imageSize = { width: mod.default.width, height: mod.default.height };
		...
		episodeMap[index].ImageSize.push(imageSize);
